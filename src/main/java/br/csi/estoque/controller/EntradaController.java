...
import br.csi.estoque.model.entrada.Entrada;
import br.csi.estoque.model.entrada.EntradaDTO;
import br.csi.estoque.service.EntradaService;
...
public class EntradaController {
    ...
    public List<EntradaDTO> listar() {
        List<Entrada> entradas = this.service.listar();
    ...
    @GetMapping("/{uuid}")
    public ResponseEntity<EntradaDTO> buscar(
        ...
        return entrada.map(e -> ResponseEntity.ok(new EntradaDTO(e)))
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Entrada cadastrada com sucesso",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = EntradaDTO.class))),
            @ApiResponse(responseCode = "400", description = "Erro na validação dos dados")
    })
    @PostMapping
    public ResponseEntity<EntradaDTO> salvar(@RequestBody @Valid Entrada entrada, UriComponentsBuilder uriBuilder) {
    ...
    @PutMapping
    public ResponseEntity<EntradaDTO> atualizarUUID(@RequestBody @Valid Entrada entrada) {
